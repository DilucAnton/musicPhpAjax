...
<body>
        ...
        <ul id="artistsList">
            <?php while ($row = $result->fetch_assoc()): ?>
                <li id="artist-<?= $row['artist_id'] ?>">
                    ...
                    <form method="post" class="restore-artist-form" data-id="<?= $row['artist_id'] ?>" style="display:inline;">
    <input type="hidden" name="restore" value="<?= $row['artist_id'] ?>">
    <button type="submit">Restore</button>
        ...
        <div id="restoreArtistSection" style="display: none;">
    <h3>Restore Artist</h3>
    <form id="restoreArtistForm">
        <input type="hidden" id="restoreArtistId" name="restore">
        <button type="submit">Restore</button>
    </form>
</div>
    ...
    $(document).ready(function() {
        ...
        $("#addArtistFormElement").submit(function(event) {
            ...
            $.post("ajax_handler.php", formData, function(response) {
                ...
                if (res.status === 'success') {
                    ...
                    //alert('Artist added successfully: ' + res.artist.pseudonym);
        ...
        $(document).on("submit", ".delete-artist-form", function(event) {
            ...
            $.post("ajax_handler.php", formData + "&action=delete_artist", function(response) {
                ...
                if (res.status === 'success') {
                    ...
                    //alert('Artist deleted successfully: ' + res.deleted_artist.pseudonym);
        ...
        $(document).on("submit", ".restore-artist-form", function(event) {
    event.preventDefault();
    var formData = $(this).serialize();
    var artistId = $(this).data("id");

    $.post("ajax_handler.php", formData + "&action=restore_artist", function(response) {
        var res = JSON.parse(response);
        if (res.status === 'success') {
            alert('Artist restored successfully: ' + res.restored_artist.pseudonym);
...
$(document).on("submit", ".restore-artist-form", function(event) {
    ...
    changesList.empty();
    var action = changes.action === 'added' ? 'was added' : 'was deleted';
    var pseudonym = changes.pseudonym ? 'Pseudonym: ' + changes.pseudonym : '';
    var realName = changes.real_name ? 'Real Name: ' + changes.real_name : '';
    var age = changes.age ? 'Age: ' + changes.age : '';
    var country = changes.country ? 'Country: ' + changes.country : '';
    var famousTrack = changes.famous_track ? 'Famous Track: ' + changes.famous_track : '';
    var songLink = changes.song_link ? 'Song Link: ' + changes.song_link : '';
    var coverLink = changes.cover_link ? 'Cover Link: ' + changes.cover_link : '';
    var about = changes.about ? 'About: ' + changes.about : '';

    var changeString = 'Artist ' + action + ': ' + pseudonym + ', ' + realName + ', ' + age + ', ' + country + ', ' + famousTrack + ', ' + songLink + ', ' + coverLink + ', ' + about;
    changesList.append('<li>' + changeString + '</li>');
}
